			...
			if (tahunAjaranAktif) {
				...
				if (guruId) {
					await prisma.jadwalPelajaran.create({
						data: { guruId, mapelId: mapelLiterasi.id, kelasId: newKelas.id, tahunAjaranId: tahunAjaranAktif.id, hari: 4, waktuMulai: "1", waktuSelesai: "-", ruang: newKelas.nama }
					});
					await prisma.jadwalPelajaran.create({
						data: { guruId, mapelId: mapelNumerasi.id, kelasId: newKelas.id, tahunAjaranId: tahunAjaranAktif.id, hari: 2, waktuMulai: "1", waktuSelesai: "-", ruang: newKelas.nama }
					});
				}
			}
		}
